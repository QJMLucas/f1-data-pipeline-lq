...
def _fetch_laps_single_period(date_start, date_end, is_final=False, session_key=None, driver_number=None, lap_number=None):
...
def get_laps(mode='incremental', session_key=None, driver_number=None, lap_number=None, date_start=None, date_end=None):
    ...
    if mode == 'backfill':
        ...
        while current_date < end_date:
            ...
            _fetch_laps_single_period(period_start, period_end, is_final=is_final, session_key=session_key, driver_number=driver_number, lap_number=lap_number)
        ...
        if date_end is None:
            date_end = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0).strftime('%Y-%m-%dT%H:%M:%S.%f+00:00')

        print('date_start >= ' + str(date_start) , 'end_time < ' + str(date_end))
        _fetch_laps_single_period(date_start, date_end, is_final=True, session_key=session_key, driver_number=driver_number, lap_number=lap_number)
